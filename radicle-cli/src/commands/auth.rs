...
pub fn init(options: Options) -> anyhow::Result<()> {
    ...
    term::info!(
        ...
        display(&term::format::command("rad clone rad:z3gqcJUoA1n9HaHKufZs5FCSGazv5"))
    );
    term::info!(
...
pub fn authenticate(options: Options, profile: &Profile) -> anyhow::Result<()> {
    if !profile.keystore.is_encrypted()? {
        term::success!("Authenticated as {}", display(&term::format::tertiary(profile.id())));
        return Ok(());
    }
    ...
    match ssh::agent::Agent::connect() {
        Ok(mut agent) => {
            ...
            term::success!("Radicle key added to {}", display(&term::format::dim("ssh-agent")));
