...
mod tests {
    ...
    #[quickcheck]
    fn prop_u8(input: u8) {
        assert_eq!(deserialize::<u8>(&input.encode_vec()).unwrap(), input);
    }

    #[quickcheck]
    fn prop_u16(input: u16) {
        assert_eq!(deserialize::<u16>(&input.encode_vec()).unwrap(), input);
    }

    #[quickcheck]
    fn prop_u32(input: u32) {
        assert_eq!(deserialize::<u32>(&input.encode_vec()).unwrap(), input);
    }

    #[quickcheck]
    fn prop_u64(input: u64) {
        assert_eq!(deserialize::<u64>(&input.encode_vec()).unwrap(), input);
    }
    ...
    fn prop_string(input: String) -> qcheck::TestResult {
        ...
        assert_eq!(deserialize::<String>(&input.encode_vec()).unwrap(), input);
    ...
    fn prop_vec(input: BoundedVec<String, 16>) {
        assert_eq!(
            deserialize::<BoundedVec<String, 16>>(&input.encode_vec()).unwrap(),
            input
        );
    }
    ...
    fn prop_pubkey(input: PublicKey) {
        assert_eq!(
            deserialize::<PublicKey>(&input.encode_vec()).unwrap(),
            input
        );
    }
    ...
    fn prop_filter(input: filter::Filter) {
        assert_eq!(
            deserialize::<filter::Filter>(&input.encode_vec()).unwrap(),
            input
        );
    }
    ...
    fn prop_id(input: RepoId) {
        assert_eq!(deserialize::<RepoId>(&input.encode_vec()).unwrap(), input);
    }

    #[quickcheck]
    fn prop_refs(input: Refs) {
        assert_eq!(deserialize::<Refs>(&input.encode_vec()).unwrap(), input);
    }

    #[quickcheck]
    fn prop_tuple(input: (String, String)) {
        assert_eq!(
            deserialize::<(String, String)>(&input.encode_vec()).unwrap(),
            input
        );
    }
    ...
    fn prop_signature(input: [u8; 64]) {
        ...
        assert_eq!(
            deserialize::<Signature>(&signature.encode_vec()).unwrap(),
            signature
        );
    }
    ...
    fn prop_oid(input: [u8; 20]) {
        ...
        assert_eq!(deserialize::<git::Oid>(&oid.encode_vec()).unwrap(), oid);
    }

    #[quickcheck]
    fn prop_signed_refs(input: SignedRefs<Unverified>) {
        assert_eq!(
            deserialize::<SignedRefs<Unverified>>(&input.encode_vec()).unwrap(),
            input
        );
    }
