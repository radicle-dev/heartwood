...
fn test_channel_reader_limit() {
    ...
    let limits = radicle::node::config::Limits {
        channels: radicle::node::config::ChannelLimits {
            reader_limit: radicle::node::config::ByteLimit::new(1000),
        },
    ...
    assert!(reason.contains("failed to read streaming pack file"));
}
