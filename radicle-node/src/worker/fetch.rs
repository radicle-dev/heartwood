...
fn set_canonical_refs(repo: &Repository, applied: &Applied) -> Result<(), error::Canonical> {
    ...
    for update in applied.updated.iter() {
        let Some(name) = (match update {
            RefUpdate::Updated { name, .. } | RefUpdate::Created { name, .. } => Some(name),
            _ => None,
        }) else {
        ...
        let Some(canonical) = (match identity.rules().canonical(name.clone(), repo) {
            Err(_) => {
                log::warn!(target: "worker", "Failed to get canonical tips for {}", name);
                None
            }
